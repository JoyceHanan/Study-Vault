...
chatApp.post("/send", verifyToken, async (req, res) => {
  try {
    ...
    await chatDoc.save();
    const populatedChat = await ChatModel.findById(chatDoc._id).populate(
      "sender",
      "name photo"
    );
...
chatApp.get("/:roomId", verifyToken, async (req, res) => {
  try {
    const chats = await ChatModel.find({ roomId: req.params.roomId })
...
// EDIT MESSAGE
chatApp.put("/:messageId", verifyToken, async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Message edited", payload: chat });
  } catch (err) {
    console.log(err);
